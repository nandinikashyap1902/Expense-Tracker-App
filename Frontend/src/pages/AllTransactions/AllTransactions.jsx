...
import { FaSortAmountDown, FaSortAmountUp, FaSearch, FaPlus, FaTrash, FaEdit } from 'react-icons/fa';
...
const AllTransactions = () => {
    ...
    useEffect(() => {
        const fetchTransactions = async () => {
            try {
                ...
                // Handle both paginated { transactions: [] } and flat [] response
    ...
    return (
        <Layout>
            ...
            <div style={{ background: 'white', padding: '1.5rem', borderRadius: '20px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', marginBottom: '2rem' }}>
                <div style={{ position: 'relative', marginBottom: '1rem' }}>
                    ...
                    <input
                        ...
                        style={{ width: '100%', padding: '0.8rem 1rem 0.8rem 2.8rem', borderRadius: '12px', border: '1px solid #e2e8f0', fontSize: '1rem', outline: 'none', boxSizing: 'border-box' }}
                    />
                </div>
            ...
            <div style={{ background: 'white', borderRadius: '20px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', padding: '1rem 1.5rem', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', fontWeight: '600', color: '#64748b' }}>
                ...
                {filteredTransactions.length === 0 ? (
                    ...
                    filteredTransactions.map(transaction => (
                        ...
                        >
                            <div style={{ color: '#64748b', fontSize: '0.9rem' }}>{formatDate(transaction.datetime)}</div>
                            <div style={{ fontWeight: 500, color: '#1e293b' }}>{transaction.description || 'No description'}</div>
                            <div>
                            ...
                            <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center' }}>
                                <button onClick={() => navigate(`/edit-transaction/${transaction._id}`)}
                                    style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: '1.1rem', padding: '0.5rem', borderRadius: '8px' }}>
                                    <FaEdit />
                                </button>
                                <button onClick={() => handleDelete(transaction._id)}
                                    style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', fontSize: '1.1rem', padding: '0.5rem', borderRadius: '8px' }}>
                                    <FaTrash />
                                </button>
                            </div>
                        </div>
